from modules.math.maths import (
    mul_down_fixed,
    div_down_fixed,
    mul_up_fixed,
    div_up_fixed,
...
def compute_add_liquidity_unbalanced(
    ...
    # Condition to check if invariant_ratio is less than _MAX_INVARIANT_RATIO
    if invariant_ratio > max_invariant_ratio:
        raise ValueError("Invariant ratio too high")
...
def compute_remove_liquidity_single_token_exact_in(
    ...
    # Condition to check if invariant_ratio is greater than the _MIN_INVARIANT_RATIO
    invariant_ratio = div_up_fixed(new_supply, total_supply)
    if invariant_ratio < min_invariant_ratio:
        raise ValueError("Invariant ratio too low")
